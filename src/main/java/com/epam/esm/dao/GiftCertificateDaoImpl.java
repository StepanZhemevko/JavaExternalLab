...
public class GiftCertificateDaoImpl implements GiftCertificateDao {
    ...
    public List<GiftCertificate> findCertificateByUserLogin(String userLogin) {
        List<GiftCertificate> certificates = jdbcTemplate.query(SQLQuery.GET_CERTIFICATE_BY_USER_LOGIN,new Object[]{userLogin}, new GiftCertificateRowMapper(jdbcTemplate));
        return certificates;
    }
    ...
    public List<GiftCertificate> findAll(int offset, int limit) {
        List<GiftCertificate> certificates = jdbcTemplate.query(SQLQuery.GET_ALL,new Object[]{limit, offset}, new GiftCertificateRowMapper(jdbcTemplate));
        return certificates;
    }
    @Override
    public List<GiftCertificate> findAllGifts() {
        List<GiftCertificate> certificates = jdbcTemplate.query(SQLQuery.GET_ALL_GIFT,new Object[]{}, new GiftCertificateRowMapper(jdbcTemplate));
        return certificates;
    }
    @Override
    ...
    public GiftCertificate findByName(String name) {
        try {
            return jdbcTemplate.queryForObject(SQLQuery.GET_CERTIFICATE_BY_NAME, new Object[]{name}, new GiftCertificateRowMapper(jdbcTemplate));
        } catch (EmptyResultDataAccessException ex) {
            return null;
        }
    }
    ...
    public void updateGiftCertificatePrice(int certificateId, double price) {
        String query = "UPDATE gift_certificate SET price = ? WHERE id = ?";
        jdbcTemplate.update(query, price, certificateId);
    }
